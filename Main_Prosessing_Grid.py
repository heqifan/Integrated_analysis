...
def Ada_R_P(mean_data, y_data, r_name):
    ...
    if r_name == 'RR':
        if np.isnan(mean_data).any() or np.isnan(y_data).any():
            ...
            model = HalvingGridSearchCV(model, param_distributions,
                                           random_state=rng,factor=2,n_jobs = -1).fit(mean_data, y_data.ravel())
            logging.info(f'-------最佳参数为: {model.best_params_}---------')
            logging.info(f'-------最佳度量值为: {model.best_score_}---------')
            logging.info(f'-------最佳模型为: {model.best_estimator_}---------')
            logging.info(f'-------交叉验证拆分（折叠/迭代）的次数为: {model.n_splits_}---------')
...
def Cal_R2(Setnodata_datas):
    ...
    for name,da in tqdm(enumerate(Setnodata_datas[:-1]),desc = 'Cal_R2'):
        ...
        try:
            pool.restart()
            mean_results = list(tqdm(pool.imap(L_R,images_pixels1,images_pixels5,name_list)))
            pool.close()
            pool.join()
        except:
            message('R2 运行成功')
...
def Mean_Median_RR(Setnodata_datas,nn_mean,nn_median):
    ...
    try:
        pool.restart()
        mean_results = list(tqdm(pool.imap(L_R,images_pixels2,images_pixels5,name_list)))
        pool.close()
        pool.join()
...
def Multiply_Regression_RR(Setnodata_datas,nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(M_R_P,images_pixels1,images_pixels5,name_list)))
    pool.close()
    pool.join()
    # message(nn + '运行成功了hhhhhhh')
    # except:
...
def Bagging_RR(Setnodata_datas, nn):
    ...
    pool.restart()
    mean_results = list(tqdm(pool.imap(Ba_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
    print('————————————————————————————————')
...
def Ada_RR(Setnodata_datas, nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(Ada_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
    # #message(nn + '运行成功了hhhhhhh')
    # except:
...
def Gra_RR(Setnodata_datas, nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(Gra_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
    #message(nn + '运行成功了hhhhhhh')
    # except:
...
def Sta_RR(Setnodata_datas, nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(Sta_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
        #message(nn + '运行成功了hhhhhhh')
    # except:
...
def RF_RR(Setnodata_datas, nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(RF_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
    # message(nn + '运行成功了hhhhhhh')
    # except:
...
def LCE_RR(Setnodata_datas, nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(LCE_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
        #message(nn + '运行成功了hhhhhhh')
    # except:
...
def Vote_RR(Setnodata_datas, nn):
    ...
    # try:
    pool.restart()
    mean_results = list(tqdm(pool.imap(Vote_R_P, images_pixels1, images_pixels5, name_list)))
    pool.close()
    pool.join()
    #message(nn + '运行成功了hhhhhhh')
    # except:
