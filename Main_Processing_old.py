...
def Cal_R2(Setnodata_datas):
    ...
    for name, da in tqdm(enumerate(Setnodata_datas[:-1]), desc='Cal_R2'):
        ...
        print('———————————R2 Pool Start—————————————————————')
        try:
            mean_results = pool.map(L_R, np.array(images_pixels1).T.tolist(), np.array(images_pixels5).T.tolist(),
                                    name_list)
...
def Mean_Median_RR(Setnodata_datas, nn_mean, nn_median):
    ...
    for year in tqdm(range(len(years)), desc='Mean,Median'):
        a = np.array(
            [Setnodata_datas[0][year], Setnodata_datas[1][year], Setnodata_datas[2][year], Setnodata_datas[3][year],
             Setnodata_datas[4][year]])
    ...
    try:
        mean_results = pool.map(L_R, np.array(images_pixels2).T.tolist(), np.array(images_pixels5).T.tolist(),
                                name_list)
...
def Weight_RR(Setnodata_datas, R2_SetNodata, nn):
    ...
    sg.popup_notify(f'Weight Task done! Spend-time: {datetime.datetime.now() - start}', display_duration_in_ms=10000,
...
def Weight_Year(Setnodata_datas, R2_SetNodata, nn):
    ...
    for year in tqdm(range(len(years)), desc='Weight Year'):
        ...
        for y in range(miny_miny):
            for x in range(minx_minx):
                a = np.array(
                    [Setnodata_datas[0][year][y][x], Setnodata_datas[1][year][y][x], Setnodata_datas[2][year][y][x],
                     Setnodata_datas[3][year][y][x], Setnodata_datas[4][year][y][x]])
                if len(a[~np.isnan(a)]) < length:
                    images_pixels3.append(np.nan)
                else:
                    images_pixels3.append((Setnodata_datas[0][year][y][x] * R2_SetNodata[0][y][x] +
                                           Setnodata_datas[1][year][y][x] * R2_SetNodata[1][y][x] +
                                           Setnodata_datas[2][year][y][x] * R2_SetNodata[2][y][x] +
                                           Setnodata_datas[3][year][y][x] * R2_SetNodata[3][y][x] +
                                           Setnodata_datas[4][year][y][x] * R2_SetNodata[4][y][x]) / (
                                                      R2_SetNodata[0][y][x] + R2_SetNodata[1][y][x] +
                                                      R2_SetNodata[2][y][x] + R2_SetNodata[3][y][x] +
                                                      R2_SetNodata[4][y][x]))
        images_pixels1.append(images_pixels3)
    A_WriteArray(images_pixels1, nn, years)
    sg.popup_notify(f'Weight_Year Task done! Spend-time: {datetime.datetime.now() - start}',
...
def normalization_Writearray_Spatial_time(Datas):
    ...
    MuSyQ_min, GLASS_min, MODIS_min, CASA_min, W_min, LAI_min = min(MuSyQ_min_list), min(GLASS_min_list), min(
        MODIS_min_list), min(CASA_min_list), min(W_min_list), min(LAI_min_list)
    MuSyQ_max, GLASS_max, MODIS_max, CASA_max, W_max, LAI_max = max(MuSyQ_max_list), max(GLASS_max_list), max(
        MODIS_max_list), max(CASA_max_list), max(W_max_list), max(LAI_max_list)
