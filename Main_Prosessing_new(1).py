...
def A_WriteArray(datalist,Name,var_list):
    ...
    img_datatype = gdal.GDT_Float16                    # 数据类型
...
def Mean_Median_RR(Setnodata_datas,nn_mean,nn_median):
    ...
    A_WriteArray([mean_results[:,i] for i in range(mean_result.shape[1])],nn_mean,var)
    del images_pixels1
    del mean_results
    gc.collect()
    print('————————————————————————————————')
    ...
    gc.collect()
    mean_results = np.array(mean_results).astype('float16')
    A_WriteArray([mean_results[:,i] for i in range(mean_result.shape[1])],nn_median,var)
    del mean_results
    del Setnodata_datas
    gc.collect()
...
def Weight_RR(Setnodata_datas,R2_SetNodata,nn):
    ...
    gc.collect()
    mean_results = np.array(mean_results).astype('float16')
    A_WriteArray([mean_results[:,i] for i in range(mean_result.shape[1])],nn,var)
    del mean_results
    del Setnodata_datas
    gc.collect()
