...
def slope(images, outpath):
    ...
    for image in images:
        ...
        band = open_tif.GetRasterBand(1).ReadAsArray().astype('float16') # 获取波段的矩阵
        band = band +  0.0001
...
for year in range(styear, edyear + 1):
    image  = inpath + os.sep + str(year) + os.sep + name + str(year) +  '.tif'
    images.append(image)
slope(images, inpath + '\\' + name + 'slope.tif')
